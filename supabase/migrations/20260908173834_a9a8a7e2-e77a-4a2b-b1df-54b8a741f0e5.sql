...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'store_subscriptions', (
      SELECT jsonb_build_object(
        'android_active', count(DISTINCT user_id) FILTER (WHERE platform = 'android' AND is_active AND expiry_time > now()),
        'ios_active', count(DISTINCT user_id) FILTER (WHERE platform = 'ios' AND is_active AND expiry_time > now()),
        'android_total', count(*) FILTER (WHERE platform = 'android'),
        'ios_total', count(*) FILTER (WHERE platform = 'ios')
      )
      FROM public.play_billing_subscriptions
    ),
    'daily', (
