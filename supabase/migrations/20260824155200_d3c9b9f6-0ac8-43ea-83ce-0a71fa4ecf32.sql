CREATE OR REPLACE FUNCTION public.get_exercises_for_student(p_ids text[])
 RETURNS TABLE(id text, lesson_id text, type text, question text, options jsonb, blanks jsonb, lines jsonb, pairs jsonb, statement text, code_template text, sort_order integer, xp integer)
 LANGUAGE plpgsql
...
BEGIN
  ...
  SELECT
    ...
    CASE WHEN jsonb_typeof(e.lines) = 'array' THEN (
      SELECT jsonb_agg(l - 'order' ORDER BY md5(auth.uid()::text || e.id || COALESCE(l->>'id', l::text)))
      FROM jsonb_array_elements(e.lines) AS l
    ) ELSE NULL END,
    e.pairs,
    e.statement,
    e.code_template,
...
BEGIN
  ...
  SELECT
    ...
    CASE WHEN jsonb_typeof(ee.lines) = 'array' THEN (
      SELECT jsonb_agg(l - 'order' ORDER BY md5(auth.uid()::text || ee.id || COALESCE(l->>'id', l::text)))
      FROM jsonb_array_elements(ee.lines) AS l
...
CREATE OR REPLACE FUNCTION public.get_test_items_for_student(p_assignment_id uuid, p_variant text)
 RETURNS TABLE(id uuid, test_id uuid, sort_order integer, source_type text, source_id text, points numeric, question text, item_type text, options jsonb, blanks jsonb, lines jsonb, pairs jsonb, statement text, code_template text)
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
BEGIN
  ...
  RETURN QUERY
  SELECT
    ti.id, ti.test_id, ti.sort_order, ti.source_type, ti.source_id, ti.points,
    ...
    CASE WHEN ti.source_type = 'custom' AND jsonb_typeof(ti.custom_data->'lines') = 'array' THEN (
      SELECT jsonb_agg(l - 'order' ORDER BY md5(auth.uid()::text || ti.id::text || COALESCE(l->>'id', l::text)))
      FROM jsonb_array_elements(ti.custom_data->'lines') AS l
    ) ELSE NULL END,
