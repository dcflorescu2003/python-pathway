ALTER TABLE public.completed_lessons ADD COLUMN IF NOT EXISTS solution_revealed_at timestamptz;

CREATE OR REPLACE FUNCTION public.award_progress(p_item_id text, p_score integer, p_allow_redo boolean DEFAULT true, p_via_solution boolean DEFAULT false, p_client_version text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  INSERT INTO public.completed_lessons (user_id, lesson_id, score, completed_at, solution_revealed_at)
  VALUES (v_uid, p_item_id, v_best_score, now(), CASE WHEN p_via_solution THEN now() ELSE NULL END)
  ON CONFLICT (user_id, lesson_id)
  DO UPDATE SET score = greatest(public.completed_lessons.score, EXCLUDED.score),
      solution_revealed_at = CASE
        WHEN p_via_solution THEN coalesce(public.completed_lessons.solution_revealed_at, now())
        ELSE public.completed_lessons.solution_revealed_at
      END;
