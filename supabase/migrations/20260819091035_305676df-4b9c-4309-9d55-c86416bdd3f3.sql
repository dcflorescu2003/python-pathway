DROP POLICY IF EXISTS "Users can delete their own completed lessons" ON public.completed_lessons;
REVOKE DELETE ON public.completed_lessons FROM authenticated;
...
DECLARE
  ...
  v_daily_cap constant integer := 1500;
...
BEGIN
  ...
  -- Plafon zilnic de siguranță, calculat din itemii finalizați azi.
  IF v_awarded > 0 THEN
    SELECT coalesce(sum(
      CASE WHEN c.lesson_id LIKE 'problem-%' THEN coalesce(pb.xp_reward, 0)
           ELSE coalesce(l.xp_reward, 0) END
    ), 0)::integer
    INTO v_today_xp
    ...
    WHERE c.user_id = v_uid AND c.completed_at >= v_today::timestamptz;

    IF v_today_xp >= v_daily_cap THEN
      v_awarded := 0;
      v_capped := true;
    ELSIF v_today_xp + v_awarded > v_daily_cap THEN
      v_awarded := v_daily_cap - v_today_xp;
      v_capped := true;
    END IF;
  END IF;
