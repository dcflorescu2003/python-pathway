...
import { useChapters, Chapter, Lesson, Exercise, ExerciseType } from "@/hooks/useChapters";
...
import { ChevronDown, ChevronRight, Edit2, Trash2, Plus, Eye, Save, X } from "lucide-react";
...
const ContentEditor = () => {
  ...
  const [chapterForm, setChapterForm] = useState({ title: "", description: "", icon: "", color: "" });
  ...
  const handleSaveExercise = async (exercise: Exercise) => {
    ...
    const row = {
      ...
      correct_option_id: exercise.correctOptionId || null,
      code_template: exercise.codeTemplate || null,
      ...
      statement: exercise.statement || null,
      is_true: exercise.isTrue ?? null,
      explanation: exercise.explanation || null,
      xp: exercise.xp,
      sort_order: isNew ? sortOrder : undefined,
    };
  ...
  // Filter out generated "Fixare" lessons (they have IDs ending in 'f')
              ...
              <div className="border-t border-border p-4 space-y-3 bg-secondary/20">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <Input value={chapterForm.title} onChange={e => setChapterForm(f => ({ ...f, title: e.target.value }))} />
                  </div>
                  <div>
                    <Label className="text-foreground text-xs">Icon</Label>
                    <Input value={chapterForm.icon} onChange={e => setChapterForm(f => ({ ...f, icon: e.target.value }))} />
                  </div>
                </div>
                <div>
                  <Label className="text-foreground text-xs">Descriere</Label>
                  <Textarea value={chapterForm.description} onChange={e => setChapterForm(f => ({ ...f, description: e.target.value }))} rows={2} />
                </div>
                      ...
                      return (
                        <div key={lesson.id} className="rounded-lg border border-border bg-secondary/30">
                          <div className="flex items-center gap-2 p-3">
                            ...
                            <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => isLessonEditing ? setEditingLesson(null) : startEditLesson(lesson)}>
                              <Edit2 className="h-3.5 w-3.5" />
                            </Button>
                            <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => navigate(`/lesson/${lesson.id}`)}>
                            ...
                            <AlertDialog>
                              ...
                              <AlertDialogContent>
                                <AlertDialogHeader>
                                  ...
                                  <AlertDialogDescription>Sigur vrei să ștergi "{lesson.title}"? Toate exercițiile vor fi pierdute.</AlertDialogDescription>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
                                  <AlertDialogCancel>Anulează</AlertDialogCancel>
                                  <AlertDialogAction onClick={() => deleteLesson(chapter.id, lesson.id)}>Șterge</AlertDialogAction>
                                </AlertDialogFooter>
                              </AlertDialogContent>
                            </AlertDialog>
                          </div>
                              ...
                              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="border-t border-border">
                                <div className="p-3 space-y-2">
                                  {lesson.exercises.map((ex, i) => (
                                    <div key={ex.id} className="flex items-center gap-2 rounded-lg border border-border bg-card p-3 text-sm">
                                      ...
                                      <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => setEditingExercise({ lessonId: lesson.id, exercise: ex })}>
                                        <Edit2 className="h-3.5 w-3.5" />
                                      </Button>
                                      <AlertDialog>
                                        <AlertDialogTrigger asChild>
                                        ...
                                        <AlertDialogContent>
                                          ...
                                          <AlertDialogFooter>
                                            <AlertDialogCancel>Anulează</AlertDialogCancel>
                                            <AlertDialogAction onClick={() => deleteExercise(ex.id)}>Șterge</AlertDialogAction>
                                          </AlertDialogFooter>
                                        </AlertDialogContent>
                                      </AlertDialog>
                                    </div>
                                  ...
                                  {editingExercise?.lessonId === lesson.id ? (
                                    ...
                                    <Button variant="outline" size="sm" className="w-full" onClick={() => setEditingExercise({ lessonId: lesson.id })}>
