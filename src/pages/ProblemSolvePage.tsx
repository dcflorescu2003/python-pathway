      ...
      <div className="px-4 py-4 space-y-4">
                ...
                if (line.startsWith("**")) {
                  return (
                    <p key={i} className="text-sm text-foreground">
                      <strong dangerouslySetInnerHTML={{ __html: line.replace(/\*\*(.*?)\*\*/g, "<strong class='text-accent'>$1</strong>") }} />
                    </p>
                  );
                }
                return (
        ...
        {problem.hint && (
          ...
          >
            ...
            {showHint ? "Ascunde indiciul" : "Arată indiciu"}
          </button>
        )}
        {showHint && problem.hint && (
          <Card className="border-warning/30 bg-warning/5">
            <CardContent className="p-3">
              <p className="text-sm text-warning">{problem.hint}</p>
            </CardContent>
          </Card>
        )}
                ...
                <CardContent className="p-3 space-y-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-medium text-foreground">
                      Test {i + 1} {result.hidden ? "(ascuns)" : ""}
                    </span>
                  </div>
                  <div className="pl-6 space-y-1">
                    <p className="text-xs text-muted-foreground font-mono">
                      Intrare: {result.input.replace(/\n/g, " ↵ ")}
                    </p>
                    <p className="text-xs text-muted-foreground font-mono">
                      Așteptat: {result.expectedOutput}
                    </p>
                    {!result.passed && (
